...
def checkIn(cwd) -> bool:
    ...
    need = ['friends_library.json', 'account_based_features.csv',
            'account_based_measures.csv']
    l = os.listdir(dpath)
    for i in need:
        if (i not in l):
            return False
...
def optList() -> int:  # feature list
    ...
    print("--> Press 7 to get \"SIR\" -Ranking Output")

    # print("--> Press 7 to get \"TOPSIS\" -Ranking Output")
    ...
    # print("--> Press 10 to plot and see \"Visual Representation\" of a User in created Social Network")
...
def doSIR(sn):
    print("\n|| SIR || selected->")
    success = sn.calculate_SIR()
    if success:
        print("SIR score calculated successfully.")
        print("Output filepath ----> ./Output/SIR_score.csv")
    else:
...
def doAAj(sn, files):
    ...
    ai_score_exists = os.path.exists("./Output/Acquaintance_score.csv")
    if ai_score_exists:
        sn.calculate_AAJ()
    else:
        print("First need to calculate Acquaintance Score A ( i )")
        success = sn.calculate_Ai_score(files[2], files['mention_count'])
        if success:
            doAAj(sn, files)
    sleep(1)
...
def doAAIj(sn, files):
    ...
    aaj_score_exists = os.path.exists(
        "./Output/Acquaintance-Affinity Score AA ( j ).csv")
    if aaj_score_exists:
        sn.calculate_AAIj()
    else:
        print("We need to calculate Acquaintance-Affinity Score AA ( j ) first...")
        doAAj(sn, files)
        doAAIj(sn, files)
    sleep(1)


def do_kshell_with_nx(sn):
    ...
    if n not in [1, 2]:
        ...
        do_kshell_with_nx(sn)
    else:
        start = time.time()
        if n == 1:
            print("Calculating K-Shell Output for In-degree...")
            sn.unweighted_k_shell_with_nx(mode="in")
        else:
            print("Calculating K-Shell Output for Out-degree...")
            sn.unweighted_k_shell_with_nx(mode="out")
        end = time.time()
        print("...Done")
        print(f"Time taken for execution is {end - start} seconds.")
...
def doKshell(sn, files):
    ...
    if opt not in [1, 2]:
        print(
            "|| Failure || : Wrong Inputs for K-Shell Ranking Selection!!...Try Again->\n")
        doKshell(sn, files)
    else:
        if opt == 1:  # UnWeighted
            print("\n|| Unweighted K-Shell Ranking || selected->")
            print("--> Press 1 to Use New Method (faster)")
            print("--> Press 2 to Use Old Method")
            ...
            if method_type == 1:
                ...
                print(
                    "--> Press 1 to get Unweighted K-Shell Output for In-degree of the network")
                print(
                    "--> Press 2 to get Unweighted K-Shell Output for Out-degree of the network")
                print("|| Enter your choice--> ", end="")
                n = int(input())
                if (n not in [1, 2]):
                    print(
                        "|| Failure || : Wrong Inputs for Unweighted K-Shell Ranking Selection!!...Try Again->\n")
                    doKshell(sn, files)
                else:
                    if (n == 1):
            ...
            print(
                "--> Press 1 to get Weighted K-Shell Output for In-degree of the network")
            print(
                "--> Press 2 to get Weighted K-Shell Output for Out-degree of the network")
            print(
                "--> Press 3 to get [Not Converted]-Weighted K-Shell Output for In-degree of the network")
            print(
                "--> Press 4 to get [Not Converted]-Weighted K-Shell Output for Out-degree of the network")
            print("|| Enter your choice--> ", end="")
            n = int(input())
            if (n not in [1, 2, 3, 4]):
                print(
                    "|| Failure || : Wrong Inputs for Weighted K-Shell Ranking Selection!!...Try Again->\n")
                doKshell(sn, files)
            else:
                if (n == 1):
...
def doTopsis(sn, files):
    ...
    if(n not in [1, 2]):
        ...
        if(n == 1):
            # outf for topsis output, inpf, outf for kshell
            sn.printCSV_topsis(
                w, mode="in", inputfile=files[3], outputfile=files[16], outf=files[17])
        else:
            # outf for topsis output, inpf, outf for kshell
            sn.printCSV_topsis(
                w, mode="out", inputfile=files[3], outputfile=files[16], outf=files[21])
...
def doPageRank(sn, files):
    ...
    sn.printCSV_Ranks(name="page", mode=1,
                      filepath=files[10], damp=d, epoch=ep)
...
def doUserRank(sn, files):
    ...
    if (opt in [1, 2]):
        ...
        print(
            "|| Failure || : Wrong Inputs for User Rank Ranking Selection!!...Try Again->\n")
        doUserRank(sn, files)
...
def doTunkRank(sn, files):
    ...
    if (opt in [1, 2]):
        ...
        if (n == 1):
            ...
            m = int(input())
        sn.printCSV_Ranks(name="tunk", mode=opt,
                          filepath=files[f], epoch=ep, mul=m)
    else:
        print(
            "|| Failure || : Wrong Inputs for Tunk Rank Ranking Selection!!...Try Again->\n")
        doTunkRank(sn, files)


def do_bcr_with_nx(sn):
    print("Calculating bcr [new method]...")
    start = time.time()
    sn.get_csv_ranks_with_nx(mode="bcr")
    end = time.time()
    print('....Done')
    print(f"Time taken for execution is {end - start} seconds.")
    time.sleep(1.2)
...
def doBCR(sn, files):
    ...
    if method_type == 1:
        ...
        if (m == 1):
            ...
            print("|| Enter your choice--> ", end="")
            n = int(input())
        sn.printCSV_Ranks(name="bcr", mode=1, xmode=m, xmode1=n,
                          sfpath=files[18], filepath=files[12])


def do_ccr_with_nx(sn):
    print("Calculating ccr [new method]...")
    start = time.time()
    sn.get_csv_ranks_with_nx(mode="ccr")
    end = time.time()
    print('....Done')
    print(f"Time taken for execution is {end - start} seconds.")
    time.sleep(1.1)
...
def doCCR(sn, files):
    ...
    if method_type == 1:
        ...
        if (m == 1):
            ...
            print("|| Enter your choice--> ", end="")
            n = int(input())
        sn.printCSV_Ranks(name="ccr", mode=1, xmode=m, xmode1=n,
                          sfpath=files[18], filepath=files[13])
...
if __name__ == "__main__":
    ...
    files = {
        # Friends json
        1: cwd + '/Data/friends_library.json'.replace('\\', '/'),
        # ACM output
        2: cwd + '/Data/account_based_features.csv'.replace('\\', '/'),
        # AVGM output
        3: cwd + '/Data/account_based_measures.csv'.replace('\\', '/'),
        'mention_count': "./Data/mentions_count.csv",

        # THE DON'T TOUCH ZONE***********************************************************************************************
        # user_name and ID
        0: cwd + '/Output/usersList.csv'.replace('\\', '/'),
        # faster input form saved network
        4: cwd + '/Output/Social Network_saved_data.json'.replace('\\', '/'),
        # user rank 1
        5: cwd + '/Output/user_rank_1_output.csv'.replace('\\', '/'),
        # user rank 2
        6: cwd + '/Output/user_rank_2_output.csv'.replace('\\', '/'),
        # user rank 3
        7: cwd + '/Output/user_rank_3_output.csv'.replace('\\', '/'),
        # tunk rank 1
        8: cwd + '/Output/tunk_rank_1_output.csv'.replace('\\', '/'),
        # tunk rank 2
        9: cwd + '/Output/tunk_rank_2_output.csv'.replace('\\', '/'),
        # page rank 1
        10: cwd + '/Output/page_rank_output.csv'.replace('\\', '/'),
        # folder path for network image
        11: cwd + '/Output/'.replace('\\', '/'),
        # Betweenness Centrality Rank 1
        12: cwd + '/Output/bcr_rank_output.csv'.replace('\\', '/').replace('\\', '/'),
        # Closeness Centrality Rank 1
        13: cwd + '/Output/ccr_rank_output.csv'.replace('\\', '/').replace('\\', '/'),
        # used in ukshell
        14: cwd + '/Output/Unweighted_K-Shell(In_Degree)_Output.csv'.replace('\\', '/'),
        # used in wkshell
        15: cwd + '/Output/Weighted_K-Shell(In_Degree)_Output.csv'.replace('\\', '/'),
